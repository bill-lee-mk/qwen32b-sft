...
#
#   # 7. 针对性改进（复用已有规则，只改 MSQ 和 Fill-in 中低于 90% 的组合）
#   QTYPE=msq,fill-in WEAK_THRESHOLD=90 RULES_RUN_ID=matrix RUN_ID=matrix_fix ROUNDS=10 bash scripts/run_matrix.sh
#
#   # 8. 全部跑完后汇总矩阵
  ...
  for GRADE in $GRADES; do
    ...
    if [ "$ROUNDS" -gt 1 ]; then
      ...
      if [ -n "$DIFFICULTY" ]; then
        LOOP_ARGS="$LOOP_ARGS --difficulty $DIFFICULTY"
      fi
      if [ -n "$WEAK_THRESHOLD" ]; then
        LOOP_ARGS="$LOOP_ARGS --weak-threshold $WEAK_THRESHOLD"
      fi
      if [ -n "$PILOT" ]; then
